...
class _CurlCffiWebSocketAdapter:
    def __init__(self, websocket: object, session: object) -> None:
...
class DexScraper:
    ...
    def __init__(
        ...
        self._curl_cffi_available = _has_curl_cffi()
    ...
    async def _connect(self) -> Optional[websockets.WebSocketServerProtocol]:
        ...
        if self.transport == "curl_cffi":
            return await self._connect_curl_cffi()

        websocket = await self._connect_websockets()
        if websocket is None and self._curl_cffi_available:
            logger.warning(
                "WebSocket transport failed after retries; falling back to curl_cffi"
            )
            return await self._connect_curl_cffi()
        return websocket

    async def _connect_websockets(
        self,
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.debug("Connection attempt %s/%s", attempt + 1, self.max_retries)
                ...
                logger.error("Connection attempt %s failed: %s", attempt + 1, exc)
    ...
    async def _connect_curl_cffi(
        self,
    ) -> Optional[websockets.WebSocketServerProtocol]:
        if not self._curl_cffi_available:
            logger.error("curl_cffi transport requested but not available")
            return None

        uri = self.config.build_websocket_url()
        logger.debug("Connecting via curl_cffi to: %s", uri)
        headers = self._get_headers()
        if self.cf_bypass:
            cf_data = await self.cf_bypass.prepare_websocket_connection(uri)
            headers = self.cf_bypass.get_enhanced_headers(headers)
        ...
        try:
            await self._rate_limit()
            return await _connect_with_curl_cffi(uri, headers)
        except Exception as exc:
            logger.error("curl_cffi connection failed: %s", exc)
            return None
    ...
    async def extract_tokens_from_message(
        self, message: bytes, max_tokens: int = 50
    ) -> ExtractedTokenBatch:
        """Extract token data from a streaming message without reconnecting."""
        if not isinstance(message, (bytes, bytearray)):
            return ExtractedTokenBatch()
        pairs_pos = message.find(b"pairs")
        if pairs_pos < 0:
            return ExtractedTokenBatch()

        data_start = pairs_pos + 20  # Validated offset
        data_section = message[data_start:]
        tokens = await self._extract_all_tokens(data_section, data_start)
        if max_tokens > 0:
...
async def _connect_with_curl_cffi(
    ...
    return _CurlCffiWebSocketAdapter(websocket, session)
